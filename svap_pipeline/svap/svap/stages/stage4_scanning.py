...
def run(storage: SVAPStorage, client: BedrockClient, run_id: str, config: dict):
    ...
    try:
        ...
        if not policies:
            # Try to extract policies from policy documents in RAG store
        ...
        for policy in policies:
            ...
            prompt = client.render_prompt(
                "stage4_score.txt",
                policy_name=policy["name"],
                structural_characterization=policy.get("structural_characterization", policy.get("description", "")),
        ...
        for r in results:
            marker = "🔴" if r["convergence_score"] >= threshold else "🟡" if r["convergence_score"] >= threshold - 1 else "🟢"
